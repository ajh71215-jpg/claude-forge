...
**Phase 3 note:** the `delegate` tool's *description* already tells Claude when to offload, so no system-prompt injection was added (keeps the prompt cache byte-stable + avoids persona conflicts). A `cheap` magic-keyword nudge + an eval quality-regression guard remain as Phase 3 follow-ups. Goal: let Forge's orchestration delegate simple/cheap subtasks to **free or cheaper non-Anthropic models** (OpenRouter `:free`, Google Gemini free tier, Groq, local Ollama) as *full agentic* sub-agents (file edit / grep / shell), so the easy work costs $0 and only failures escalate to paid Claude.
